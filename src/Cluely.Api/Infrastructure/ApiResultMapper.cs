...
public static class ApiResultMapper
{
    ...
    private static int MapStatusCode(Error error)
    {
        return error switch
        {
            ...
            BusinessError { Code: "InvalidExternalToken" } => StatusCodes.Status401Unauthorized,
            BusinessError { Code: "ExpiredExternalToken" } => StatusCodes.Status401Unauthorized,
            BusinessError { Code: "WrongExternalAudience" } => StatusCodes.Status401Unauthorized,
            BusinessError { Code: "WrongExternalIssuer" } => StatusCodes.Status401Unauthorized,
            BusinessError { Code: "ProviderUnavailable" } => StatusCodes.Status503ServiceUnavailable,
