...
public sealed class PublishDictionaryHandler
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            publishResult = dictionary.Publish(owner, versionId, publishedAt);
        }
        ...
        var pendingEvents = dictionary.GetPendingEvents();

        if (pendingEvents.Count > 0)
        {
            await _dictionaryRepository.UpdateAsync(dictionary, cancellationToken);
            await _eventPublisher.PublishAsync(pendingEvents, cancellationToken);
            dictionary.ClearPendingEvents();
        }

        return Result.Success(new PublishDictionaryResult(
            command.DictionaryId,
            publishResult.VersionId.Value,
            publishResult.VersionLabel,
            publishResult.WordCount));
    }
}
