...
public sealed class ShareDictionaryHandler
{
    private readonly IDictionaryRepository _dictionaryRepository;
    private readonly IDomainEventPublisher _eventPublisher;
    private readonly ICurrentUserAccessor _currentUserAccessor;
    private readonly IValidator<ShareDictionaryCommand> _validator;

    public ShareDictionaryHandler(
        IDictionaryRepository dictionaryRepository,
        IDomainEventPublisher eventPublisher,
        ICurrentUserAccessor currentUserAccessor,
        IValidator<ShareDictionaryCommand> validator)
    {
    ...
    public async Task<Result<ShareDictionaryResult>> HandleAsync(
        ShareDictionaryCommand command,
        CancellationToken cancellationToken = default)
    {
        var validationResult = await _validator.ValidateAsync(command, cancellationToken);
        if (!validationResult.IsValid)
        {
            return Result.Failure<ShareDictionaryResult>(new ValidationError(
                "ValidationFailed",
        ...
        {
            return Result.Failure<ShareDictionaryResult>(new BusinessError(
                "Unauthorized",
        ...
        {
            return Result.Failure<ShareDictionaryResult>(new BusinessError(ex.GetType().Name, ex.Message));
        }
        catch (Exception ex)
        {
            return Result.Failure<ShareDictionaryResult>(new UnexpectedError(
                "UnexpectedError",
        ...
        return Result.Success(new ShareDictionaryResult(command.DictionaryId, command.GranteeId));
    }
}
