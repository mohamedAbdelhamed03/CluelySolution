...
Every implementation review updates this file. Last reviewed: **Backend Release Candidate RC1, 2026-07-12**.
...
| TD-004 | Publish exception consistency | Slice 04 | Required | Closed | Publish validation and domain failures use the shared Result/Error mapping; unexpected failures are sanitized by global ProblemDetails handling. | feature/backend-release-candidate |
...
| TD-011 | Draft validation aggregate-version semantics | Slice 04 | Required | Closed | Validation is transient and does not advance aggregate version; `ValidateDraftHandler` therefore does not persist an unchanged aggregate version, and publish revalidates the draft. | feature/content-api |
...
| TD-021 | Concurrent refresh-token replay | Backend RC1 | Blocking | Closed | Refresh rotation now conditionally revokes and inserts the replacement in one SQL transaction; concurrent replay allows exactly one success. | feature/backend-release-candidate |
| TD-022 | Moderator authorization placeholder | Backend RC1 | Blocking | Closed | Replaced the deny-all placeholder with a deployment-controlled moderator user-ID allow-list behind the existing Application port. | feature/backend-release-candidate |
