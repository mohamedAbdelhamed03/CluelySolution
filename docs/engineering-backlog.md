...
| TD-018 | Moderator retire-version path | Slice 04 workflow | Required | Open | FR-CONTENT-084 allows a moderator (not only the owner) to retire a version, but the domain exposes only `RetireVersion(OwnerId, VersionId)`. The `RetireVersion` handler is therefore owner-only; a domain `RetireVersion(ModeratorId, VersionId)` overload plus moderator-seam handler enforcement is required to complete the moderation lifecycle. Recorded rather than redesigning the frozen domain. (Number may need reconciliation with the publishing branch's TD-017.) | — |
